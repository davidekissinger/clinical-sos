...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    // Fetch regulatory signals for this facility
    ...
    if (!signals || signals.length === 0) {
      return Response.json({
        ...
        regulatory_urgency_score: 0,
        commercial_opportunity_score: 0,
        clinical_sos_priority_score: 0,
        regulatory_urgency: 'Unknown / Research Required',
    ...
    for (const sig of signals) {
      ...
      switch (sig.signal_type) {
        case 'Immediate Jeopardy':
          weight = ruWeights.immediate_jeopardy ?? 35;
          label = 'Immediate Jeopardy';
          break;
        case 'CMP':
          weight = (sig.cmp_type === 'Per Day')
            ? (ruWeights.cmp_per_day ?? 30)
            : (sig.cmp_type === 'Per Instance')
              ? (ruWeights.cmp_per_instance ?? 20)
              : (ruWeights.cmp_per_instance ?? 20);
          label = `CMP (${sig.cmp_type || 'Unknown'})`;
          break;
        case 'DPNA':
          weight = ruWeights.dpna ?? 28;
          label = 'Denial of Payment for New Admissions';
          break;
        case 'Special Focus Facility':
          weight = ruWeights.sff_active ?? 25;
          label = 'Special Focus Facility';
          break;
        case 'SFF Candidate':
          weight = ruWeights.sff_candidate ?? 18;
          label = 'SFF Candidate';
          break;
        case 'Repeat Deficiency':
          weight = ruWeights.repeat_serious_deficiency ?? 18;
          label = 'Repeat Deficiency';
          break;
        case 'Follow-Up Survey':
          weight = (sig.revisit_status === 'Failed') ? (ruWeights.failed_revisit ?? 22) : (ruWeights.upcoming_revisit ?? 12);
          label = `Follow-Up Survey (${sig.revisit_status || 'Pending'})`;
          break;
        case 'Infection-Control Deficiency':
          weight = ruWeights.infection_control ?? 15;
          label = 'Infection-Control Deficiency';
          break;
        default:
          weight = sig.severity === 'High' ? (ruWeights.high_severity ?? 12) : sig.severity === 'Medium' ? (ruWeights.medium_severity ?? 8) : (ruWeights.low_severity ?? 4);
      ...
      ruBreakdown.push({
        ...
        weight,
        recency_multiplier: Math.round(recencyMultiplier * 100) / 100,
        severity_multiplier: severityMultiplier,
        verification_multiplier: verifiedMultiplier,
        points,
        source: sig.source,
    ...
    if (facility.operator_name) {
      ...
      if (operatorFacilityCount > 1) {
        const multiBonus = Math.min(10, operatorFacilityCount * 2);
        ...
        ruBreakdown.push({
          ...
          label: `Multi-facility operator (${operatorFacilityCount} facilities)`,
          weight: ruWeights.multi_facility_operator_bonus ?? 5,
          recency_multiplier: 1,
          severity_multiplier: 1,
          verification_multiplier: 1,
          points: bonusPoints,
          source: 'Internal facility database',
          evidence_summary: `${facility.operator_name} operates ${operatorFacilityCount} facilities — enforcement at one may indicate systemic risk.`,
        });
      }
    }
    ...
    // Determine regulatory urgency classification
    ...
    if (!hasVerified && ruScore < 40) {
      regulatoryUrgency = 'Unknown / Research Required';
    } else if (hasIJ || hasDPNA || hasCMPPerDay || (highSeverityCount >= 3 && hasVerified)) {
      regulatoryUrgency = 'Critical';
    } else if (ruScore >= 70) {
      regulatoryUrgency = 'Severe';
    } else if (ruScore >= 50) {
      regulatoryUrgency = 'High';
    } else if (ruScore >= 30) {
      regulatoryUrgency = 'Moderate';
    } else {
      regulatoryUrgency = 'Proactive';
    }
    ...
    // Service match (based on signals → recommended service)
    const hasEnforcementSignals = signals.some(s => ['Immediate Jeopardy', 'CMP', 'DPNA', 'Special Focus Facility'].includes(s.signal_type));
    if (hasEnforcementSignals) {
      coScore += coWeights.service_match ?? 20;
      coBreakdown.push({ label: 'Strong Clinical SOS service match (enforcement recovery)', points: coWeights.service_match ?? 20 });
    } else {
      coScore += (coWeights.service_match ?? 20) * 0.5;
      coBreakdown.push({ label: 'Moderate service match (consulting/proactive)', points: Math.round((coWeights.service_match ?? 20) * 0.5) });
    }

    // Operator size
    if (operatorFacilityCount >= 5) {
      coScore += coWeights.operator_size ?? 15;
      coBreakdown.push({ label: `Multi-facility operator (${operatorFacilityCount} facilities)`, points: coWeights.operator_size ?? 15 });
    } else if (operatorFacilityCount >= 2) {
      coScore += (coWeights.operator_size ?? 15) * 0.7;
      coBreakdown.push({ label: `Small operator (${operatorFacilityCount} facilities)`, points: Math.round((coWeights.operator_size ?? 15) * 0.7) });
    }

    // Multi-fility affected
    if (operatorFacilityCount > 1) {
      coScore += coWeights.multi_facility_affected ?? 10;
      coBreakdown.push({ label: 'Multiple facilities potentially affected', points: coWeights.multi_facility_affected ?? 10 });
    }

    // Bed count (operator size proxy)
    if (facility.bed_count && facility.bed_count >= 100) {
      coScore += 8;
      coBreakdown.push({ label: `Large facility (${facility.bed_count} beds)`, points: 8 });
    } else if (facility.bed_count && facility.bed_count >= 50) {
      coScore += 5;
      coBreakdown.push({ label: `Medium facility (${facility.bed_count} beds)`, points: 5 });
    }
    ...
    if (contacts && contacts.length > 0) {
      ...
      if (hasExec) {
        coScore += coWeights.decision_maker_identified ?? 15;
        coBreakdown.push({ label: 'Regional/executive decision-maker identified', points: coWeights.decision_maker_identified ?? 15 });
      } else {
        coScore += (coWeights.decision_maker_identified ?? 15) * 0.6;
        coBreakdown.push({ label: 'Facility-level contact identified (administrator/DON)', points: Math.round((coWeights.decision_maker_identified ?? 15) * 0.6) });
      }
    }

    // Urgency alignment
    if (regulatoryUrgency === 'Critical' || regulatoryUrgency === 'Severe') {
      coScore += coWeights.urgency_alignment ?? 12;
      coBreakdown.push({ label: 'High regulatory urgency aligns with immediate need', points: coWeights.urgency_alignment ?? 12 });
    } else if (regulatoryUrgency === 'High') {
      coScore += (coWeights.urgency_alignment ?? 12) * 0.7;
      coBreakdown.push({ label: 'Moderate urgency alignment', points: Math.round((coWeights.urgency_alignment ?? 12) * 0.7) });
    }

    // Prior interaction
    const interactions = await svc.entities.Interaction.filter({ facility_id: facilityId });
    if (interactions && interactions.length > 0) {
      coScore += coWeights.prior_interaction ?? 8;
      coBreakdown.push({ label: 'Prior interaction recorded', points: coWeights.prior_interaction ?? 8 });
    }

    coScore = Math.min(100, Math.round(coScore));

    // ============ CLINICAL SOS PRIORITY SCORE ============
    ...
    const leadData = {
      ...
      lead_score: priorityScore,
      lead_tier: priorityScore >= 75 ? 'Tier 1' : priorityScore >= 55 ? 'Tier 2' : priorityScore >= 40 ? 'Tier 3' : 'Nurture',
      score_explanation: explanation,
      regulatory_urgency: regulatoryUrgency,
      regulatory_urgency_score: ruScore,
      commercial_opportunity_score: coScore,
      clinical_sos_priority_score: priorityScore,
    ...
    if (existingLeads && existingLeads.length > 0) {
      lead = await svc.entities.Lead.update(existingLeads[0].id, leadData);
    } else {
      lead = await svc.entities.Lead.create(leadData);
    }

    // Update facility with scores
    await svc.entities.Facility.update(facilityId, {
      regulatory_urgency: regulatoryUrgency,
      regulatory_urgency_score: ruScore,
      commercial_opportunity_score: coScore,
      clinical_sos_priority_score: priorityScore,
    ...
    return Response.json({
      ...
      regulatory_urgency: regulatoryUrgency,
      explanation,
      regulatory_breakdown: ruBreakdown,
      commercial_breakdown: coBreakdown,
      signals_count: signals.length,
      verified_count: signals.filter(s => s.verified).length,
      operator_facility_count: operatorFacilityCount,
      lead_id: lead?.id,
    });
  } catch (error) {
