...
const STATUS_ICON = { PASS: CheckCircle2, WARNING: AlertTriangle, FAIL: XCircle };
const STATUS_TONE = { PASS: "green", WARNING: "amber", FAIL: "red" };
const STATUS_COLOR = { PASS: "text-emerald-600", WARNING: "text-amber-600", FAIL: "text-rose-600" };
...
export default function LaunchReadiness() {
  ...
  const criticalWarnings = critical.filter((c) => c.status === "WARNING");
  const productionReady = criticalFails.length === 0;
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Production readiness acceptance testing — all critical security, privacy, verification, and communication checks must pass"
        action={
      ...
      <div className={`rounded-2xl border-2 p-6 mb-6 ${productionReady ? "border-emerald-300 bg-emerald-50" : "border-rose-300 bg-rose-50"}`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className={`text-3xl font-bold ${productionReady ? "text-emerald-700" : "text-rose-700"}`}>
              {productionReady ? "YES" : "NO"}
            </p>
            {productionReady ? (
              <p className="text-sm text-emerald-600 mt-1">All critical checks passed. System is cleared for production use.</p>
            ) : (
              <p className="text-sm text-rose-600 mt-1">
                {criticalFails.length} critical failure{criticalFails.length !== 1 ? "s" : ""} must be resolved before launch.
              </p>
            )}
          </div>
        </div>
              ...
              {catChecks.map((c) => {
                ...
                return (
                  <div key={c.id} className="bg-white rounded-xl border border-border p-4 flex items-start gap-4">
                    <Icon className={`h-5 w-5 flex-shrink-0 mt-0.5 ${STATUS_COLOR[c.status]}`} />
                    <div className="flex-1 min-w-0">
                      ...
                      {c.unresolved_issue && (
                        <p className="mt-1 text-sm text-rose-600 font-medium">⚠ {c.unresolved_issue}</p>
                      )}
                      <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                        {c.last_tested_date && <span>Last tested: {new Date(c.last_tested_date).toLocaleString()}</span>}
